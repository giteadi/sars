...
export const fetchPropertyAvailabilityById = createAsyncThunk(
  ...
  async (propertyId, { rejectWithValue }) => {
    try {
      const response = await axios.get(`https://api.sarsdecor.com/api/v1/check/chekAvialibilityById/${propertyId}`);
...
export const addPropertyAvailability = createAsyncThunk(
  ...
  async ({ property_id, start_date, end_date, is_booked = 0, number_of_guests, user_id }, { rejectWithValue }) => {
    try {
      const response = await axios.post('https://api.sarsdecor.com/api/v1/check/addAvialibility', {
        property_id,
        start_date,
        end_date,
        is_booked,
  ...
  async ({ selectedStartDate, selectedEndDate, propertyId }, { rejectWithValue }) => {
    try {
      ...
      const reservationResponse = await axios.get(
        `https://api.sarsdecor.com/api/v1/reservation/getReservationByPropertyID/${propertyId}`
      );
